...
class RelatorioServicoController {
  ...
  // Método para criar relatório pelo prestador
  static async createByPrestador(req, res) {
    try {
      const { descricao, ordemServicoId, custo_total } = req.body
      ...
      // Obter a empresa do prestador
      const empresaId = prestador.empresaId
      if (!empresaId) {
        return res.status(400).json({ error: "Prestador não está associado a uma empresa" })
      }
      ...
      // Verificar se a ordem de serviço pertence à mesma empresa do prestador
      if (ordemServico.empresaId !== empresaId) {
        return res.status(403).json({ error: "A ordem de serviço não pertence à mesma empresa do prestador" })
      }
  ...
  // Método para listar relatórios por prestador
  static async listByPrestador(req, res) {
    try {
      ...
      // Buscar relatórios de serviço associados ao prestador
      const relatorios = await RelatorioServico.findAll({
        where: { prestadorId },
        include: [
          {
      ...
      console.error(error)
      return res.status(500).json({ error: "Erro ao listar relatórios de serviço do prestador" })
    }
  }
